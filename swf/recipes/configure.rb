...
node[:deploy].each do |app_name, deploy|
  ...
  execute "launch rake task 'workflow_worker' in background " do
    ...
    command "RAILS_ENV=#{deploy[:rails_env]} BACKGROUND=y PIDFILE=/srv/www/#{deploy[:application]}/shared/rake-workflow_worker.pid bundle exec rake swf:workflow_worker"
    action :run
  end

  execute "launch rake task 'activity_worker' in background " do
    cwd deploy[:current_path]
    command "RAILS_ENV=#{deploy[:rails_env]} BACKGROUND=y PIDFILE=/srv/www/#{deploy[:application]}/shared/rake-activity_worker.pid bundle exec rake swf:activity_worker"
    action :run
  end
